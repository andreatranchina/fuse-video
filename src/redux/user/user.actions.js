...
export const fetchUserByIdThunk = (id) => {
  return async (dispatch) => {
    try {
      let res = await axios.get(`https://video-backend-6mkl.onrender.com/api/user/${id}`);
...
export const editAccountThunk = (id, editedAccount) => {
  return async (dispatch) => {
    ...
    const newAccountInfo = await axios.put(`https://video-backend-6mkl.onrender.com/api/user/${id}`, {
...
export const fetchUserByEmailThunk = (userEmail) => {
  return async (dispatch) => {
    ...
    user = await axios.get(`https://video-backend-6mkl.onrender.com/api/user/byEmail/${userEmail}`)
    console.log('made edit account axios call')
...
export const fetchFollowersThunk = (userId) => {
  return async(dispatch) => {
    try {
      const res = await axios.get(`https://video-backend-6mkl.onrender.com/api/follows/followers/${userId}`)
...
export const fetchFollowingsThunk = (userId) => {
  return async(dispatch) => {
    try {
      const res = await axios.get(`https://video-backend-6mkl.onrender.com/api/follows/followings/${userId}`)
...
export const addProfileToViewsThunk = (viewUserId,loggedInUserId) => {
  return async(dispatch) => {
    try {
      const res = await axios.get(`https://video-backend-6mkl.onrender.com/api/user/viewProfile/${viewUserId}?loggedInUserId=${loggedInUserId}`);
